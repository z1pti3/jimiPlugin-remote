...
class _remoteConnectLinux(action._action):
    ...
    def doAction(self,data):
        host = helpers.evalString(self.host,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        user = helpers.evalString(self.user,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        remote_port = helpers.evalString(self.remote_port,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        if not remote_port:
            remote_port = 22
        if self.password.startswith("ENC") and self.password != "":
            password = auth.getPasswordFromENC(self.password)
        elif "%%" in self.password:
            password = helpers.evalString(self.password,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        else:
            password = ""
        keyfile = helpers.evalString(self.keyfile,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
...
class _remoteConnectWindows(action._action):
    ...
    def doAction(self,data):
        ...
        if self.check_hostname:
            hostname = helpers.evalString(self.hostname,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        else:
            hostname = ""
...
class _remoteConnectFortigate(action._action):
    ...
    def doAction(self,data):
        host = helpers.evalString(self.host,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        port = helpers.evalString(self.port,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        if not port:
            port = 22
        deviceHostname = helpers.evalString(self.deviceHostname,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        user = helpers.evalString(self.user,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        if self.password.startswith("ENC"):
            password = auth.getPasswordFromENC(self.password)
        elif "%%" in self.password:
...
class _remoteConnectCisco(action._action):
    ...
    def doAction(self,data):
        ...
        if not port:
            port = 22
        deviceHostname = helpers.evalString(self.deviceHostname,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        ssh_username = helpers.evalString(self.ssh_username,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        if self.ssh_password.startswith("ENC"):
            ssh_password = auth.getPasswordFromENC(self.ssh_password)
        elif "%%" in self.ssh_password:
            ssh_password = helpers.evalString(self.ssh_password,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        else:
            ssh_password = ""
        if self.enable_password.startswith("ENC"):
            ...
            enable_password = helpers.evalString(self.enable_password,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        else:
            enable_password = ""
        ...
        if client.client != None:
            ...
            return {"result" : True, "rc" : 0, "msg" : f"Connection successful", "connection_id" : connection_id}
        else:
            return {"result" : False, "rc" : 403, "msg" : "Connection failed - {0}".format(client.error)}
...
class _remoteMultiCommand(action._action):
    ...
    def doAction(self,data):
        ...
        commandResults = []
        commands = helpers.evalString(self.commands,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        try:
            client = data["eventData"]["remote"][connection_id]["client"]
        except KeyError:
            return {"result" : False, "rc" : 255, "msg" : "Unable to load remote connection - connection_id='{0}'".format(connection_id)}
...
class _remoteDownload(action._action):
    ...
    def doAction(self,data):
        ...
        remoteFile = helpers.evalString(self.remoteFile,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
        localFile = helpers.evalString(self.localFile,{"data" : data["flowData"], "eventData" : data["eventData"], "conductData" : data["conductData"], "persistentData" :  data["persistentData"] })
